...
const RiskManagement = () => {
  ...
  const content: Record<
    string,
    {
      ...
      risks: { icon: string; text: string; fullWidth?: boolean }[];
    }
  > = {
    "tabungan-abdi": {
      title: "",
  ...
  return (
    <div className="container mx-auto px-4">
      <h4 className="text-primary text-2xl font-semibold mb-8">
          ...
          {currentContent.benefits.map((benefit, index) => (
            <div
              ...
              className={`flex items-start space-x-4 ${
                benefit.text === "Setoran awal yang terjangkau."
                  ? "ml-[-100px]"
                  : ""
              }`}
            >
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {currentContent.risks.map((risk, index) => (
            <div
              key={index}
              className={`flex items-start space-x-4 ${
                risk.fullWidth === true ? "md:col-span-3" : ""
              }`}
            >
              <div className="w-12 h-12 flex-shrink-0">
                <Image
