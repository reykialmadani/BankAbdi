...
const Header = () => {
  ...
  const getSubmenuContent = (menu: MenuType): MenuItem[] => {
    const submenuMap: MenuMap = {
      ...
      laporan: [
        { title: "Laporan Bulanan", href: "/laporan/bulanan", desc: "Laporan bulanan BANK ABDI yang berisi tentang aktivitas" },
        { title: "Laporan Tahunan", href: "/laporan/tahunan", desc: "Laporan tahunan BANK ABDI yang berisi tentang aktivitas" },
        { title: "Laporan Publikasi", href: "/laporan/publikasi", desc: "Laporan publikasi BANK ABDI untuk transparansi informasi" },
        { title: "Laporan Tata Kelola", href: "/laporan/tata-kelola", desc: "Laporan tata kelola BANK ABDI dalam menjalankan perusahaan" },
      ],
    };
    return submenuMap[menu] || [];
  };

  const renderSubmenu = (menu: MenuType) => {
    ...
    return (
      <div className="grid grid-cols-2 gap-2 px-2">
        <h2 className="text-lg font-semibold text-gray-900 flex items-center gap-2 capitalize col-span-2">
          {menu}
          <Image src="https://bankabdi.co.id/img/icon/arrow_circle_right.png" width={16} height={16} alt="arrow right" />
        </h2>
        {/* Left Column */}
            ...
            <Link key={index} href={item.href}>
              <div className="rounded-lg p-3 hover:bg-blue-50 transition-colors duration-200">
                <div className="flex items-center gap-2 mb-1">
                  ...
                  <Image src="https://bankabdi.co.id/img/icon/arrow_right.png" width={16} height={16} alt="arrow right" className="flex-shrink-0" />
                </div>
      ...
      <nav className="container mx-auto flex items-center justify-between font-sogeo text-base h-full">
        ...
        <ul className="hidden md:flex space-x-6 text-gray-800">
                ...
                {(['pinjaman', 'tabungan', 'deposito'] as ProductMenuType[]).map((category) => (
                  <div key={category} className="relative group/submenu">
                    ...
                    <div className="hidden group-hover/submenu:block absolute left-full top-0 bg-white shadow-lg z-50 min-w-[600px]">
                      <div className="p-4">
                        {renderSubmenu(category as MenuType)}
                      </div>
                    </div>
                  </div>
                ))}
          ...
          <li className="relative group">
            ...
            {/* Dropdown - Full width style from old version */}
      ...
      {isMobileMenuOpen && (
        <div className="md:hidden fixed top-[60px] left-0 w-full bg-white shadow-lg z-40 max-h-[calc(100vh-60px)] overflow-y-auto">
          <div className="container mx-auto py-4 space-y-4 flex flex-col items-center">
            ...
            {/* Laporan Menu for Mobile */}
            <div className="w-full max-w-md">
              <button
                className="w-full text-center py-2 text-gray-800 hover:text-blue-600 capitalize flex items-center justify-center gap-2"
                onClick={() => handleMobileSubmenuToggle('laporan')}
              >
                <span className="font-medium">Laporan</span>
                {openMobileSubmenu === 'laporan' ? <ChevronUp className="w-4 h-4" /> : <ChevronDown className="w-4 h-4" />}
              </button>
              {openMobileSubmenu === 'laporan' && (
                <div className="py-2 space-y-2">
                  {renderSubmenu('laporan')}
                </div>
              )}
